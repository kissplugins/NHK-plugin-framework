...
define('NHK_EVENT_MANAGER_VERSION', '1.0.5');
 ...
 * For this demo, we'll create a minimal framework structure.
 */
if (!class_exists('NHK\\Framework\\Container\\Container')) {
    // Create minimal framework structure for demo
    require_once NHK_EVENT_MANAGER_PATH . 'framework/Container.php';
}
...
function nhk_event_manager_activate() {
    ...
    // Mark that sample data should be imported on next init
...
function nhk_event_manager_action_links($links) {
    $settings_link = sprintf(
        '<a href="%s">%s</a>',
        admin_url('edit.php?post_type=nhk_event&page=nhk-event-manager'),
        __('Demo & Settings', 'nhk-event-manager')
    );

    $demo_link = sprintf(
        '<a href="%s">%s</a>',
        admin_url('edit.php?post_type=nhk_event&page=nhk-event-demo'),
    ...
    array_unshift($links, $settings_link, $demo_link, $docs_link);
